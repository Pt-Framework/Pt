...
- `%onBindOptions(const StyleOptions&)` returns the overlay after `%bind(&global)`. It does not call `%prepare`. Shared style prototypes stay synchronized through `%Style::reset()` / `%onReset(...)`.
...
- `%ButtonStyler` and `%PanelStyler` do not track `_hasOverrides` or `_overridesGeneration`. Token-only changes with the same default-options mode are option changes and prepare the current renderer in place. Default-options mode flips rebind.
- `%onStyleRenderer(...)` returns `%style.get<XRenderer>()`. `%onCreateRenderer(...)` returns `%create()` of that prototype, or 0.
- In the style-path bind, clone and locally prepare a private renderer only when the overlay is not `%isDefault(global)`. If it is default, bind the shared renderer from `%Style` directly and do not prepare it locally.
- When a bind or update path cannot obtain a renderer, keep the cached prepare generations invalid. Only store the current prepare generations after a successful `%Renderer::prepare(...)` call.
