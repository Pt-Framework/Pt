...
#ifndef PT_FORMS_GDI_GRAPHICSBACKEND_H
#define PT_FORMS_GDI_GRAPHICSBACKEND_H
...
class GdiGraphicsBackend : public GraphicsBackend
{
    public:
        IPixmapImpl* createPixmapImpl() override
        {
            return new GdiPixmapImpl();
        }
        ...
        HDC bitmapContext(const Pixmap& pixmap) const
        {
            const GdiPixmapImpl* impl = static_cast<const GdiPixmapImpl*>(pixmap.impl());
            if( ! impl)
                return 0;
