...
{
    // proxied to a pthread worker via PROXY_TO_PTHREAD, so this may block normally
    while( _active )
    {
        // native blocking below does not implicitly drain proxied browser events
...
{
    // unblocks a pending SDL_WaitEventTimeout in onRun()
    SDL_Event ev = {};
