...
void BorderLayout::update()
{
    ...
    if (_south != 0) {
        ...
        _south->resize(size_t(std::max(0, right - left)), _south->preferredSize().height());
    ...
    if (_east != 0) {
        ...
        _east->resize(preferredSize.width(), size_t(std::max(0, bottom - top)));
        right -= _east->size().width() + _spacing;
    }

    if (_west != 0) {
        _west->move(left, top);
        _west->resize(_west->preferredSize().width(), size_t(std::max(0, bottom - top)));
        left += _west->size().width() + _spacing;
    }

    if (_center != 0) {
        _center->move(left, top);
        _center->resize(size_t(std::max(0, right - left)), size_t(std::max(0, bottom - top)));
    }
}
...
Math::Size BorderLayout::minimumSize()
{
    ...
    if (_center != 0) {
        ...
        middleMaxHeight = size_t(std::max(0, ssize_t(_center->minimumSize().height())));
    }
