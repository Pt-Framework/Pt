...
//#include "tests/MemoryBlockTest.h"
...
void MemoryBlock::init( std::size_t blockSize, Pt::uint8_t blocks )
{
    assert(blockSize > 0);
    assert(blocks > 0);
...
void MemoryBlock::reset(std::size_t blockSize, Pt::uint8_t blocks)
{
    assert(blockSize > 0);
    assert(blocks > 0);
    // Overflow check
...
void MemoryBlock::deallocate(void* p, std::size_t blockSize)
{
    ...
    // block more than once causes MemoryBlock's linked-list of stealth indexes to
...
// MemoryBlock::IsCorrupt -----------------------------------------------------------
#ifndef NDEBUG
bool MemoryBlock::isCorrupt(Pt::uint8_t numBlocks, std::size_t blockSize, bool checkIndexes) const
{
    if(numBlocks < _blocksAvailable)
    {
        // Contents at this MemoryBlock corrupted.  This might mean something has
        // overwritten memory owned by the MemoryBlocks container.
        assert( false );
        return true;
    }
    ...
    /* The loop goes along singly linked-list of stealth indexes and makes sure
     ...
     Contents of allocated blocks are not changed by MemoryBlock.
     ...
     Type 1: Index is too big.
      ...
      MemoryBlock's allocated domain.
...
bool MemoryBlock::isBlockAvailable(void* p, Pt::uint8_t numBlocks, std::size_t blockSize) const
{
    (void) numBlocks;
