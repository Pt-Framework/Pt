...
int pushValue(lua_State* L, Context* ctx, Pt::Any& value, Pt::Reflex::Type& type)
{
  ...
  if(*ti == typeid(bool))
  {
    lua_pushboolean(L, *static_cast<bool*>(value.get()) ? 1 : 0);
    return 1;
  }
  if(*ti == typeid(int))
  {
    lua_pushinteger(L, *static_cast<int*>(value.get()));
    return 1;
  }
  if(*ti == typeid(long))
  {
    lua_pushinteger(L, *static_cast<long*>(value.get()));
    return 1;
  }
  if(*ti == typeid(double))
  {
    lua_pushnumber(L, *static_cast<double*>(value.get()));
    return 1;
  }
  if(*ti == typeid(float))
  {
    lua_pushnumber(L, *static_cast<float*>(value.get()));
    return 1;
  }
...
int Context::pushTo(lua_State* co, Pt::Any& value, Pt::Reflex::Type& type)
{
  return pushValue(co, this, value, type);
}


void Context::pushObject(void* instance, Pt::Reflex::Type& type)
{
  LuaObjectHeader* hdr =
    static_cast<LuaObjectHeader*>(lua_newuserdatauv(_L, sizeof(LuaObjectHeader), 0));
  hdr->instance   = instance;
  hdr->type       = &type;
  hdr->destructor = 0;

  luaL_getmetatable(_L, type.name().c_str());
  lua_setmetatable(_L, -2);
}


int Context::push(Pt::Any& value, Pt::Reflex::Type& type)
{
  const std::type_info* ti = type.id();
  ...
  if(*ti == typeid(bool))
  {
    lua_pushboolean(_L, *static_cast<bool*>(value.get()) ? 1 : 0);
    return 1;
  }
  if(*ti == typeid(int))
  {
    lua_pushinteger(_L, *static_cast<int*>(value.get()));
    return 1;
  }
  if(*ti == typeid(long))
  {
    lua_pushinteger(_L, *static_cast<long*>(value.get()));
    return 1;
  }
  if(*ti == typeid(double))
  {
    lua_pushnumber(_L, *static_cast<double*>(value.get()));
    return 1;
  }
  if(*ti == typeid(float))
  {
    lua_pushnumber(_L, *static_cast<float*>(value.get()));
    return 1;
  }
  if(*ti == typeid(std::string))
  {
  ...
  // Object type: copy-construct into owned Lua userdata.
  void (*dtor)(void*) = 0;
  const std::vector<TypeManager::TypeBinding>& bindings = _tm.bindings();
  for(std::size_t i = 0; i < bindings.size(); ++i)
  {
    if(bindings[i].type == &type)
    {
  ...
  std::size_t totalSize = LUAOBJECT_DATA_OFFSET + type.size();
  LuaObjectHeader* hdr =
    static_cast<LuaObjectHeader*>(lua_newuserdatauv(_L, totalSize, 0));
  hdr->instance   = static_cast<char*>(static_cast<void*>(hdr)) + LUAOBJECT_DATA_OFFSET;
  hdr->type       = &type;
  hdr->destructor = dtor;
  std::memcpy(hdr->instance, value.get(), type.size());
