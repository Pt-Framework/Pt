...
#endif
#ifdef PTV_GEOMETRY_OUTPUT
  int hasFragmentInfos;
  fragmentDesc* fragments;
  u32 nbFragments;
#endif
};

#ifdef PTV_GEOMETRY_OUTPUT
u32 getFragmentCount(BtCursor* aCursor) 
{
    return aCursor->nbFragments;
}

fragmentDesc *getFragments(BtCursor* aCursor)
{
    return aCursor->fragments;
}

u16 getPageSize(BtCursor* aCursor)
{
    return aCursor->pPage->pBt->pageSize;
}

u32 getPageNo(BtCursor* aCursor)
{
    return aCursor->pPage->pgno;
}

int hasFragmentInfos(BtCursor* aCursor)
{
    return aCursor->hasFragmentInfos;
}

void setHasFragmentInfos(BtCursor* aCursor)
{
    aCursor->hasFragmentInfos = 1;
}

void resetHasFragmentInfos(BtCursor* aCursor)
{
    aCursor->hasFragmentInfos = 0;
}
...
//typedef struct fragmentDesc fragmentDesc;
...
static int getPayload(
  ...
  u32 aCellOffset;
  u32 aPageNo;
  u32 allocatedFragments;

  allocatedFragments = 0;
  if (pCur->hasFragmentInfos) {
    free(pCur->fragments);
    pCur->fragments = malloc(sizeof(fragmentDesc) * 10);
    allocatedFragments = 10;
    pCur->nbFragments = 0;
  }
  ...
  if( offset<pCur->info.nLocal ){ // the offset is inside the current page
    int a = amt;
    ...
    if (pCur->hasFragmentInfos)
    {
        u16 payloadHeaderLength;
        // code for determining position of data in file
        pCur->nbFragments = 1;
        aPageNo = pCur->pPage->pgno;
        aPayloadOffset = aPayload - pCur->info.pCell;
        payloadHeaderLength = *(pCur->info.pCell + pCur->info.nHeader);
        payloadHeaderLength += 4;
        aCellOffset = get2byte(pCur->pPage->aData + (2 * pCur->idx) + pCur->pPage->cellOffset) + payloadHeaderLength;
        pCur->fragments[0].aPageNo = aPageNo;
        pCur->fragments[0].aPayloadOffset = aPayloadOffset;
        pCur->fragments[0].aCellOffset = aCellOffset;
        pCur->fragments[0].aSize = a;
    }
  ...
  if( amt>0 ){
    ...
    while( amt>0 && nextPage ){
      ...
      if( offset<ovflSize ){
        ...
        if (pCur->hasFragmentInfos && allocatedFragments == pCur->nbFragments)
        {
            fragmentDesc* newFragments = malloc(sizeof(fragmentDesc) * (allocatedFragments * 2));
            memcpy(newFragments, pCur->fragments, allocatedFragments * sizeof(fragmentDesc));
            free(pCur->fragments);
            allocatedFragments *= 2;
            pCur->fragments = newFragments;
        }
        ...
        if (pCur->hasFragmentInfos) 
        {
            aCellOffset = offset;
            aPayloadOffset = 4;
            pCur->fragments[pCur->nbFragments].aPageNo = currentPage;
            pCur->fragments[pCur->nbFragments].aPayloadOffset = aPayloadOffset;
            pCur->fragments[pCur->nbFragments].aCellOffset = aCellOffset;
            pCur->fragments[pCur->nbFragments].aSize = a;
            pCur->nbFragments++;
        }
...
#ifdef PTV_GEOMETRY_OUTPUT
  // code for determining fragment information
  if (pCur->hasFragmentInfos) {
      u16 payloadHeaderLength;
      free(pCur->fragments);
      pCur->fragments = malloc(sizeof(fragmentDesc));
      pCur->nbFragments = 1;
      payloadHeaderLength = *(pCur->info.pCell + pCur->info.nHeader);
      payloadHeaderLength += 4;
      pCur->fragments[0].aPageNo = pCur->pPage->pgno;
      pCur->fragments[0].aPayloadOffset = pCur->info.nHeader;
      pCur->fragments[0].aCellOffset = get2byte(pCur->pPage->aData + (2 * pCur->idx) + pCur->pPage->cellOffset) + payloadHeaderLength;
      pCur->fragments[0].aSize = *pAmt;
  }
#endif
