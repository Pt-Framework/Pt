...
// The PTV_MM_DB define is for MemoryMaped DB, if this is not defined DiskFile is used
#define PTV_MM_DB
#undef PTV_MM_DB

// The PTV_MM_DB_WITH_VIEWS define is for MemoryMaped DB with View Support
// which fixes the Memory Use to 5 Views a 1MByte
#define PTV_MM_DB_WITH_VIEWS
//#undef PTV_MM_DB_WITH_VIEWS
...
/*
** A Struct to Hold the MemoryMap View Infos
*/
...
const int PtvMmViewCount=30;
const int PtvMMViewSize=64*1024;
...
*/
int sqlite3WinOpenReadWriteMMFile(
  ...
  winFile f;
  HANDLE h;
  // OG Needed variables for Memory Mapping
  HANDLE hMap;
  LPVOID lpAddr;
  SYSTEM_INFO SystemInfo;
  DWORD dwFileSize,dwViewAddr;
  int cnt;
  winMMViewInfo *tmp;
  // OG End
  WCHAR *zWide = utf8ToUnicode(zFilename);
...
** *id and *pReadonly unchanged.
*/
int sqlite3WinOpenReadWriteDiskFile(
  const char *zFilename,
  OsFile **pId,
  int *pReadonly
){
...
#ifdef PTV_MM_DB
  return sqlite3WinOpenReadWriteMMFile(zFilename,pId,pReadonly);
#else
  return sqlite3WinOpenReadWriteDiskFile(zFilename,pId,pReadonly);
#endif
...
#ifdef PTV_MM_DB
  // Memory Maped Version is not designed to be write some Data to the DB-File
  return SQLITE_FULL;
#endif
