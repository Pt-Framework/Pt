...
case OP_Column: {
  ...
  */
  pC = p->apCsr[p1];
#ifdef PTV_GEOMETRY_OUTPUT
  setHasFragmentInfos(pC->pCursor);
#endif
  assert( pC!=0 );
  if( pC->pCursor!=0 ){
    ...
    if( pC->nullRow ){
      ...
      zRec = (char*)pC->aRow;
#ifdef PTV_GEOMETRY_OUTPUT
      monoliticBlock = 2; // information about block was already sent to output as block is now cached.
#endif
    }else if( pC->isIndex ){
      i64 payloadSize64;
  ...
  */
  if( pC && pC->cacheStatus==p->cacheCtr ){
    ...
    if( zRec ){
      zData = zRec;
#ifdef PTV_GEOMETRY_OUTPUT
      monoliticBlock = 2; // means, that this record was already read - nothing to display.
#endif
    }else{
      if( pC->isIndex ){
  ...
  */
  if( aOffset[p2] ){
    ...
    if( zRec ){
      ...
      if (hasFragmentInfos(pCrsr) == 1) {
          u32 fragmentIndex;
          fragmentDesc *fragments;
          u32 fragmentCount;
          u16 pageSize = getPageSize(pCrsr);
          fragments = (fragmentDesc*)getFragments(pCrsr);
          fragmentCount = getFragmentCount(pCrsr);

          if (fragmentCount > 1) {
              //printf("\nTile-Info: Type: Scattered, Fragments:%d\n", fragmentCount);
              for (fragmentIndex = 0; fragmentIndex < fragmentCount; ++fragmentIndex)
              {
                  pageAddress = ((fragments[fragmentIndex].aPageNo - 1) * pageSize);
                  tileAddress = pageAddress + fragments[fragmentIndex].aCellOffset + fragments[fragmentIndex].aPayloadOffset;
                  printf("    Fragment %d Size %d Address %d\n", fragmentIndex, fragments[fragmentIndex].aSize, tileAddress);
              }
          }
      }
#endif
...
#ifdef PTV_GEOMETRY_OUTPUT
  if (hasFragmentInfos(pCrsr) && (monoliticBlock == 1)) 
  {
      //extern u32 getPageNo(BtCursor*);
      //extern u16 getPageSize(BtCursor*);
      u32 pageNo;
      fragmentDesc* fragments;
      u32 fragmentCount;
      u16 pageSize = getPageSize(pCrsr);
      fragments = getFragments(pCrsr);
      fragmentCount = getFragmentCount(pCrsr);
      pageNo = getPageNo(pCrsr);
      pageAddress = ((pageNo - 1) * pageSize);
      tileAddress = pageAddress + fragments[0].aCellOffset + fragments[0].aPayloadOffset;
      printf("    Fragment 0 Size %d Address %d\n", fragments[0].aSize-17, tileAddress);
      //printf("\nTile-Info: Type: OneBlock, Address:%d, Size:%d\n", tileAddress, fragments[0].aSize-17);
  }
#endif
...
op_column_out:
#ifdef PTV_GEOMETRY_OUTPUT
  resetHasFragmentInfos(pC->pCursor);
#endif
  break;
}
