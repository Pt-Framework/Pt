...
void FillPolygon::draw( Image& image, const Brush& brush, const PointF* points_, size_t pointCount )
{
    std::vector<PointF> points( pointCount );
    std::memcpy( &points[0], points_ , sizeof( PointF) * pointCount );
    ...
    _clipper(points, RectF( PointF(0,0), SizeF( image.width(), image.height() )) );
