...
void OverlappedHandler::onReady(HANDLE h)
{ 
	_impl->onOverlapped(h); 
}


void WakeHandler::onReady(HANDLE h)
{ 
	_impl->onWake(h); 
}


void TimerHandler::onReady()
{ 
	_impl->processTimers(); 
}


ApplicationImpl::ApplicationImpl(Signal<const Pt::Event&>& eventSignal)
: Pt::System::EventLoop()
, _event(eventSignal)
, _ovhandler()
, _selector()
, _notifier( _selector.overlappedEvent() )
, _notifier2( _selector.wakeEvent() )
, _qapp(0)
{
	_ovhandler.init(this);
    QObject::connect(&_notifier, SIGNAL(activated(HANDLE)), &_ovhandler, SLOT(onReady(HANDLE)));

	_wakeHandler.init(this);
    QObject::connect(&_notifier2, SIGNAL(activated(HANDLE)), &_wakeHandler, SLOT(onReady(HANDLE)));

	_timerHandler.init(this);
	QObject::connect(&_masterTimer, SIGNAL(timeout()), &_timerHandler, SLOT(onReady()));
