...
void Rasterizer2::adjustXWAlphas(Pt::uint8_t& a1, Pt::uint8_t& a2)
{
    ...
    lprintf("\n    };\n\n");
    lprintf("    static const Pt::uint8_t aaAF30[256] = {\n        ");
    for(int c = 0, i = 0; i <= 255; ++i) {
        const int a = i + i * 3 / 10; ++c;
        lprintf("%3d%c ", (a > 255) ? 255 : a, (i == 255) ? ' ' : ',');
        if(c >= 20) { c = 0; lprintf("\n        "); }
    }
    lprintf("\n    };\n");
    ...
    /*
    // Using equation is about 6% to 10% slower
    if(abs(a2 - a1) < 25) {
        const uint16_t aa1 = a1 + (uint16_t) a1 * 3 / 10;
        const uint16_t aa2 = a2 + (uint16_t) a2 * 3 / 10;
        a1 = (aa1 >= 255) ? 255 : aa1;
        a2 = (aa2 >= 255) ? 255 : aa2;
    }
    ...
    static const Pt::uint8_t aaAF30[256] = {
          0,   1,   2,   3,   5,   6,   7,   9,  10,  11,  13,  14,  15,  16,  18,  19,  20,  22,  23,  24,
         26,  27,  28,  29,  31,  32,  33,  35,  36,  37,  39,  40,  41,  42,  44,  45,  46,  48,  49,  50,
         52,  53,  54,  55,  57,  58,  59,  61,  62,  63,  65,  66,  67,  68,  70,  71,  72,  74,  75,  76,
         78,  79,  80,  81,  83,  84,  85,  87,  88,  89,  91,  92,  93,  94,  96,  97,  98, 100, 101, 102,
        104, 105, 106, 107, 109, 110, 111, 113, 114, 115, 117, 118, 119, 120, 122, 123, 124, 126, 127, 128,
        130, 131, 132, 133, 135, 136, 137, 139, 140, 141, 143, 144, 145, 146, 148, 149, 150, 152, 153, 154,
        156, 157, 158, 159, 161, 162, 163, 165, 166, 167, 169, 170, 171, 172, 174, 175, 176, 178, 179, 180,
        182, 183, 184, 185, 187, 188, 189, 191, 192, 193, 195, 196, 197, 198, 200, 201, 202, 204, 205, 206,
        208, 209, 210, 211, 213, 214, 215, 217, 218, 219, 221, 222, 223, 224, 226, 227, 228, 230, 231, 232,
        234, 235, 236, 237, 239, 240, 241, 243, 244, 245, 247, 248, 249, 250, 252, 253, 254, 255, 255, 255,
        255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
        255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
        255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255
    };
