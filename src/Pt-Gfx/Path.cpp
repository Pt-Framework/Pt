...
void Path::toPoints(std::vector<PointF>& dst, float smoothness) const
{
    // State variables
    double curX = 0.0;
    double curY = 0.0;
    ...
    for(it = _elements.begin(); it != _elements.end(); ++it) 
    {
        ...
        switch(ins.type) 
        {
            ...
            case Element::IT_QuadBezierTo:
                quadraticBezierToPoints(dst, curX, curY, ins.pxy[0], ins.pxy[1], ins.pxy[2], ins.pxy[3], smoothness);
                curX = ins.pxy[2];
                curY = ins.pxy[3];
                break;

            case Element::IT_CubicBezierTo:
                cubicBezierToPoints(dst, curX, curY, ins.pxy[0], ins.pxy[1], ins.pxy[2], ins.pxy[3], ins.pxy[4], ins.pxy[5], smoothness);
                curX = ins.pxy[4];
                curY = ins.pxy[5];
                break;

            case Element::IT_GenNBezierTo:
                bezierToPoints(dst, curX, curY, ins.pxy, smoothness);
    ...
    if( ! dst.empty() && 
        dst.back().x() > Painter::MaximumCoordinateF && 
        dst.back().y() > Painter::MaximumCoordinateF)
    {
        dst.pop_back();
    }
}
