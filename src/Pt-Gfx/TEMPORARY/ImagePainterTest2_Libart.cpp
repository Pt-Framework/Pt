...
static size_t libartBenchFillPolygon(int loopCount, CompositionMode cm, bool aa)
{
    ...
    for(int i = 0; i < loopCount ; ++i) {
        ...
        BENCHMARK_LIBART_DISPLAY_RESULTING_IMAGE;
    }
...
static void libartBenchmark(CompositionMode cm)
{
    double time1, time2;
    ...
    if(BENCHMARK_SOLID_FILLED_POLYGON) {
        // art_rgb_svp_alpha()
        time1 = libartBenchFillPolygon             (BENCHMARK_LOOP_COUNT, cm, false);
        ...
        std::clog << std::endl;
        // art_rgb_svp_aa()
        time1 = libartBenchFillPolygon             (BENCHMARK_LOOP_COUNT, cm, true);
...
#undef BENCHMARK_LIBART_DISPLAY_RESULTING_IMAGE
