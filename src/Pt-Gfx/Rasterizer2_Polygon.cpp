...
// http://www.realtimerendering.com/resources/GraphicsGems/category.html
    ...
    #define MAX_X     0x7FFF              // Subpixel X beyond the right edge
...
inline void vLerp(double alpha, const Point* Va, const Point* Vb, Point* Vout)
{
    ...
    Vout->setX(alpha * Va->x() + ialpha * Vb->x());
    Vout->setY(alpha * Va->y() + ialpha * Vb->y());
}
    ...
    // Subpixel X extremes for scanline

    int xLmin, xLmax;
    int xRmax, xRmin; // Optimization shortcut
...
int Coverage(int x /* left subpixel of pixel */)
{
...
void Rasterizer2::rasterPolygonAreaFastAASC(const Point* points, size_t pointCount, const Color& color, Pt::int32_t minX, Pt::int32_t minY, Pt::int32_t maxX, Pt::int32_t maxY)
{
    ...
    // Current left edge
    const Point* Vleft;
    const Point* VnextLeft;

    // Current right edge
    const Point* Vright;
    const Point* VnextRight;

    // End of polygon vertex list
    const Point* endPoly;
    ...
    #define screenX(X) (X->x())
#define LERP(W, B, A) ( (1.0-(W)) * (A) + (W) * (B) )


    // Find vertex with minimum Y
    Vleft = points;    for(int i = 1; i < pointCount; ++i) {
        if(points[i].y() < Vleft->y()) Vleft = &points[i];
    }
