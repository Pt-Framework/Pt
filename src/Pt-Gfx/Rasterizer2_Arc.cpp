...
void Rasterizer2::arcUtil_findExactBegEndPointsCoordinate(FilledArcInfo& fai)
{
    ...
    const Pt::int32_t bx = round(fai.ctrX + fai.radX * ::cosf(rBeg));
    const Pt::int32_t by = round(fai.ctrY - fai.radY * ::sinf(rBeg)); // Sign inversion due to differences between cartesian and computer coordinate systems
    ...
    const Pt::int32_t ex = round(fai.ctrX + fai.radX * ::cosf(rEnd));
    const Pt::int32_t ey = round(fai.ctrY - fai.radY * ::sinf(rEnd)); // Sign inversion due to differences between cartesian and computer coordinate systems
