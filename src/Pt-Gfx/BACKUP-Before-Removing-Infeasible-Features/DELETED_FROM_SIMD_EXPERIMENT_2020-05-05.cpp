            ...
            for(size_t j = 0; j < loop; ++j) {
                ...
                for(size_t b = 0; b <= bOne; ++b) {
                    ...
                    */
                    ...
                    // Get the Y coordinates
                    float gcurYi[4] = { points[pi + 0].y(),
                                        points[pi + 2].y(),
                                        points[pi + 4].y(),
                                        points[pi + 6].y() };

                    float gcurYj[4] = { points[pj + 0].y(),
                                        points[pj + 2].y(),
                                        points[pj + 4].y(),
                                        points[pj + 6].y() };

                    __m128 curYi = _mm_load_ps(gcurYi);
                    __m128 curYj = _mm_load_ps(gcurYj);

                    // Get the X coordinates
                    float gcurXi[4] = { points[pi + 0].x(),
                                        points[pi + 2].x(),
                                        points[pi + 4].x(),
                                        points[pi + 6].x() };

                    float gcurXj[4] = { points[pj + 0].x(),
                                        points[pj + 2].x(),
                                        points[pj + 4].x(),
                                        points[pj + 6].x() };

                    __m128 curXi = _mm_load_ps(gcurXi);
                    __m128 curXj = _mm_load_ps(gcurXj);

                    // Compare againts the Y coordinates
                    // ( curY >= curYi && curY < curYj ) || ( curY >= curYj && curY < curYi )
                    __m128 cmpYs = _mm_or_ps(
                        _mm_and_ps( _mm_cmpnlt_ps(curY, curYi), _mm_cmplt_ps(curY, curYj) ),
                        _mm_and_ps( _mm_cmpnlt_ps(curY, curYj), _mm_cmplt_ps(curY, curYi) )
                    );

                    uint32_t i_cmpYs[4];
                    _mm_store_ps( (float*) i_cmpYs, cmpYs );

                    // Calculate the interpolated X coordinates
                    __m128 deltaYp = _mm_sub_ps( curY , curYi );
                    __m128 deltaYj = _mm_sub_ps( curYj, curYi );
                    __m128 deltaXj = _mm_sub_ps( curXj, curXi );
                    __m128 interXf = _mm_add_ps( curXi, _mm_mul_ps( _mm_div_ps(deltaYp, deltaYj), deltaXj ) );

                    float f_interXf[4];
                    _mm_store_ps(f_interXf, interXf);
                    ...
                    // Store the coordinates as needed
                    if(i_cmpYs[0]) nodeX[nodes++] = f_interXf[0];
                    if(i_cmpYs[1]) nodeX[nodes++] = f_interXf[1];
                    if(i_cmpYs[2]) nodeX[nodes++] = f_interXf[2];
                    if(i_cmpYs[3]) nodeX[nodes++] = f_interXf[3];
                }
            }
