...
void Rasterizer2::strokeEllipse( const Point& topLeft, const Size& size )
{
    ...
    while( y >= 0 && x <= a )
    {
        ...
        if( x!=0 && y!=0 )
        {
            stroke( xc+x -errorx, yc-y );
            stroke( xc-x, yc+y -errory );
        }
...
void Rasterizer2::drawWideDashPolyline( const Point* pPts, int npt )
{
    int       x1, y1, x2, y2;
        ...
        if( x1 != x2 || y1 != y2 )
        {
            ...
            if( _pen.style() == Pen::DoubleDash || (startPaintType != 0))
            {
                ...
                else
                {
                    ...
                    lineJoin( &leftFace, &prevRightFace );
                }
            }
...
void Rasterizer2::dashSegment( int *pDashNum, int *pDashIndex, int *pDashOffset, int x1, int y1, int x2, int y2, bool projectLeft, bool projectRight, LineFace *leftFace, LineFace *rightFace,  int* dash )
{
  int                   dashNum, dashIndex, dashRemain;
                ...
                switch( _pen.capStyle() )
                {
                    ...
                    case Pen::TriangularCap:
                        if( !first )
                        {
                            ...
                            lineArc( &lcapFace, (LineFace *) NULL, lcenterx, lcentery, false);
                        }
                        ...
                        // right end of dash
                        lineArc((LineFace *)NULL, &rcapFace, rcenterx, rcentery, false);
                    break;
                }
            }
        }
    ...
    if( _pen.style() == Pen::DoubleDash || !(paintType == 0))
    {
        ...
        if( ( !first && ( _pen.style() == Pen::Dash) && ( _pen.capStyle() == Pen::RoundCap) ) || ( _pen.capStyle() == Pen::TriangularCap) )
        {
            ...
            lineArc( &lcapFace, (LineFace *) 0, rcenterx, rcentery, false);
        }
    }
...
void Rasterizer2::drawSegment( Point from, Point to, bool projectLeft, bool projectRight, LineFace* leftFace, LineFace* rightFace )
{
    double   l, L, r;
    double   xa, ya;
...
void Rasterizer2::drawWideSolidPolyline( const  Point* pPts, int npt )
{
  int          x1, y1, x2, y2;
    bool       projectLeft, projectRight;
    LineFace   leftFace, rightFace, prevRightFace;
    ...
    while( --npt )
    {
        ...
        if (x1 != x2 || y1 != y2)
        {
            ...
            drawSegment(  Point(x1, y1), Point(x2, y2), projectLeft, projectRight, &leftFace, &rightFace );
...
void Rasterizer2::stepDash( int dist, int* pDashNum, int* pDashIndex, const  int* pDash, int numInDashList, int *pDashOffset )
{
...
void Rasterizer2::drawThinDashPolyline( const Point* points,  int pointCount )
{
    ...
    while (--pointCount)
    {
        ...
        // horizontal or vertical lines, unlike the undashed case)
        {
            ...
            bresenhamDasheLineSegment(&dashNum, &dashIndex, &dashes[0], dashes.size(), &dashOffset, isDoubleDash, signdx, signdy, axis, x1, y1, e, e1, e2, len);
        }
    }
...
void Rasterizer2::bresenhamDasheLineSegment(int *pdashNum, int *pdashIndex, const  int *pDash, int numInDashList, int *pdashOffset, bool isDoubleDash, int signdx, int signdy, int axis, int x1, int y1, int e, int e1, int e2, int len)
{
...
void Rasterizer2::stroke( const Point& pixel)
{
...
void Rasterizer2::fillTexture(const Point& origin, const Point& pos,  int length )
{
...
void Rasterizer2::createGradientTexture(Image& texture, int width, int height,Pt::Gfx::Color gradientStart,  Pt::Gfx::Color gradientStop,  Pt::Gfx::Brush::FillStyle style)
{
...
void Rasterizer2::fillVerticalGradient( const Point& origin, const Point& pos,  int length )
{
    fillTexture(origin, pos, length);
}

void Rasterizer2::fillHorizontalGradient( const Point& origin, const Point& pos,  int length )
{
    fillTexture(origin, pos, length);
}


void Rasterizer2::fillSolid(const Point& pos, int length)
{
    int xpos = pos.x();
    int ypos = pos.y();
...
void Rasterizer2::fill(const Point& origin, const Point& pos, int length)
{
...
void Rasterizer2::clipStepEdge( int ybase, int& xcl, int& xcr, int& edgey,  LineEdge* edge, bool edgeleft )
{
  if (ybase != edgey)
      return;
...
int Rasterizer2::polyBuildPoly( const Point *vertices, const LineSlope *slopes, int count, int xi, int yi, LineEdge *left, LineEdge *right, int *pnleft, int *pnright, int *h)
{
    int     top, bottom;
    double  miny, maxy;
    int     i;
...
int Rasterizer2::buildLineEdge( double x0, double y0, double k, int dx, int dy, int xi, int yi, bool left, LineEdge *edge )
{
    int x, y, e;
    int xady;
...
void Rasterizer2::fillSpans(int x, int y,  int w,  int h)
{
...
void Rasterizer2::fillLine(int y,  int overall_height, LineEdge *left, LineEdge *right, int left_count, int right_count)
{
    int left_x      = 0;
    int left_e      = 0;
    int left_stepx  = 0;
...
void Rasterizer2::roundJoinClip (LineFace *pLeft, LineFace *pRight, LineEdge *edge1, LineEdge *edge2, int *y1, int *y2, bool *left1, bool *left2)
{
    int denom;
...
void Rasterizer2::lineArc( LineFace *leftFace, LineFace *rightFace, double xorg, double yorg, bool isInt )
{
...
int Rasterizer2::lineArcI( int xorg, int yorg, std::vector<Point>& points, std::vector<int>& widths )
{
    Point *tpts, *bpts;
    int* twids, *bwids;
    int x, y, e, ex;
...
int Rasterizer2::lineArcD( double xorg, double yorg, std::vector<Point>& points, std::vector<int>& widths, LineEdge *edge1, int edgey1, bool edgeleft1, LineEdge *edge2, int edgey2, bool edgeleft2)
{
    Point *pts;
    int *wids;
...
int Rasterizer2::roundCapClip( const LineFace *face, bool isInt, LineEdge *edge, bool *leftEdge )
{
    int     y;
    int     dx, dy;
    double  xa, ya, k;
...
int Rasterizer2::roundJoinFace( const LineFace *face, LineEdge *edge, bool *leftEdge )
{
    int     y;
    int     dx, dy;
    double  xa, ya;
...
void Rasterizer2::lineJoin(  LineFace *pLeft, LineFace *pRight )
{
    ...
    { // invoke miLineArc to fill the round join, isInt = true-
        lineArc( pLeft, pRight, (double)0.0, (double)0.0, true );
        return;
    }
    ...
    fillLine( y, height, left, right, nleft, nright);
}


/* Paint a projecting rectangular cap on a line face.  Called only by
   miWideDash (with isInt = true); not by miWideLine. */
void Rasterizer2::lineProjectingCap( const LineFace *face, bool isLeft, bool isInt )
{
    int         xorgi = 0, yorgi = 0;
    int         lw;
    LineEdge    lefts[2], rights[2];
...
void Rasterizer2::updateGradientBrush(int width, int height)
{
...
void Rasterizer2::fillRect(const Rect& rectIn)
{
...
void Rasterizer2::fill( const Point* pts, size_t pointCount)
{
...
void Rasterizer2::outputEdges(const ActiveEdgeTable& edges, const Point&  origin, int scanLine)
{
    // fill every even span, starting at even (even-odd-rule)
...
void Rasterizer2::outputSpan( const Point& topLeft, int x, int y, int width )
{
...
void Rasterizer2::fillEllipse( const Point& topLeftIn, const Size& size )
{
