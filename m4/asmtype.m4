...
AC_DEFUN([AC_CHECKATOMICTYPE],
  [
    if test "$ac_pt_atomicity" = "$1" -o "$ac_pt_atomicity" = "probe"
    then
      AC_COMPILE_IFELSE(
        [AC_LANG_SOURCE([$3])],
        PT_ATOMICITY=$2
        ac_pt_atomicity=$1,
        if test "$ac_pt_atomicity" = "$1"
        then
...
[
  ...
  AC_ARG_WITH(
    ...
    AS_HELP_STRING([--with-atomictype],
                   [force atomic type. Accepted arguments:
                    ...
                    generic, probe]),
    [ ac_pt_atomicity=$withval ],
    [ ac_pt_atomicity=probe ])

  dnl check, if atomictype is valid

  dnl sun
  AC_CHECKATOMICTYPE([sun], [PT_ATOMICITY_SUN],
      [ #include <sys/atomic.h>
        int main () { volatile ulong_t* uvalue; atomic_inc_ulong_nv( uvalue ); } ])
  AC_CHECKATOMICTYPE([windows], [PT_ATOMICITY_WINDOWS],
      [ #define _WINSOCKAPI_
        #include <windows.h>
        int main() { LONG value; InterlockedIncrement(&value); } ])
  AC_CHECKATOMICTYPE([att_arm], [PT_ATOMICITY_GCC_ARM],
      [
        #include <csignal>
  ...
  AC_CHECKATOMICTYPE([att_mips], [PT_ATOMICITY_GCC_MIPS],
      [
        #include <csignal>
        typedef std::sig_atomic_t atomic_t;
  ...
  AC_CHECKATOMICTYPE([att_ppc], [PT_ATOMICITY_GCC_PPC],
      [
        #include <csignal>
        typedef std::sig_atomic_t atomic_t;
        void atomicIncrement(volatile atomic_t& val)
  ...
  AC_CHECKATOMICTYPE([att_sparc], [PT_ATOMICITY_GCC_SPARC],
      [
        #include <csignal>
        typedef std::sig_atomic_t atomic_t;
        void atomicIncrement(volatile atomic_t& val)
  ...
  AC_CHECKATOMICTYPE([att_x86_64], [PT_ATOMICITY_GCC_X86_64],
      [ #include <unistd.h>
  ...
  AC_CHECKATOMICTYPE([att_x86], [PT_ATOMICITY_GCC_X86],
      [ #include <csignal>
        typedef std::sig_atomic_t atomic_t;
        void atomicIncrement(volatile atomic_t& val)
        {
  ...
  AC_CHECKATOMICTYPE([att_avr32], [PT_ATOMICITY_GCC_AVR32],
      [ 
  ...
  if test "$ac_pt_atomicity" = "pthread"
  then
    PT_ATOMICITY=PT_ATOMICITY_PTHREAD
  fi
  if test "$ac_pt_atomicity" = "generic"
  then
    PT_ATOMICITY=PT_ATOMICITY_GENERIC
  fi
  if test "$PT_ATOMICITY" = ""
  then
    AC_MSG_ERROR([check for atomictype failed])
  else
    AC_MSG_RESULT($ac_pt_atomicity)
  fi
])
