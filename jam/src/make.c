...
*/
    ...
    /* Step 3d: add dependencies for logging */
    
    for( c = t->dependents; c; c = c->next )
    {
        if(( c->target->flags & T_FLAG_LOGTOFILE ) && !( t->flags & T_FLAG_LOGTOFILE))
        {
            t->flags = t->flags | c->target->flags;
            t->logname = newstr(c->target->name); /* never freed */
        }
    }
...
dependGraphOutput( TARGET *t, int depth )
{
    ...
    if( t->flags & T_FLAG_LOGTOFILE ) printf ("LOGTOFILE ");
    printf( "\n" );
    }
