	...
	LIST	*shell )
{
    ...
    strcpy(commandBuffer, rule->actions->command);
    if (logFile != 0)
    {
        char* newline;
        newline = strrchr(commandBuffer, '\n');
        if (newline != 0)
            *newline = '\0';
        strcat(commandBuffer, logFile->string);
    }
    do
    {
        ...
        allocated = var_string( commandBuffer/*rule->actions->command*/, cmd->buf, max_line, &cmd->args );
