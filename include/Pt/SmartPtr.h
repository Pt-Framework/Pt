...
namespace Pt
{
	...
	*/
	template <typename T>
	/** \brief Smart pointer policy that implements reference linking.
		...
		list. When the list becomes empty the raw pointer si deleted.
	*/
	class RefLinked
	{
		private:
	...
	*/
	template <typename T>
	/** \brief Smart pointer policy that implements intrusive reference counting.
	...
	*/
	...
	/**
		\param T Type the smart pointer points at
		\param Model Model for linking/unlinking 
	*/
	template <typename T,
	          typename Model = RefLinked<T> >
