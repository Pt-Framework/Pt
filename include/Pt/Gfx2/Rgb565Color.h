	...
	namespace Gfx {
		...
		class PT_API PT_PACKED Color<Rgb565> {
			public:
				 ...
				 */
				inline Color<Rgb565>(const Color<Rgb565>& c)
				: _val(c._val)
				{}

				/** @brief Construct color using the given packed color constant.
				 */
				inline Color<Rgb565>(uint16_t val)
				: _val(val)
				{}

				/** @brief Construct color using the given components.
				 */
				inline Color<Rgb565>(uint8_t r, uint8_t g, uint8_t b)
		 ...
		 */
		inline const Color<Rgb565>& greyscale(Color<Rgb565>& to, const Color<Rgb565>& from)
		{
