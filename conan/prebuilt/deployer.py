...
def _write_packages_jam(path, prebuilt_id, packages):
    ...
    lines = [
        "# Generated by conan/prebuilt/deployer.py -- do not edit.",
        "if ! $(PREBUILT_PACKAGES_JAM_INCLUDED) {",
        "PREBUILT_PACKAGES_JAM_INCLUDED = true ;",
        "",
        "PREBUILT_SCHEMA  = 1 ;",
        "PREBUILT_ID      = {} ;".format(prebuilt_id),
        "PREBUILT_LINKAGE = shared ;",
        "",
        "PREBUILT_PACKAGES = {} ;".format(" ".join(ordered)),
        "",
    ]
...
def deploy(graph, output_folder, **kwargs):
    prebuilt_id = os.environ.get("PREBUILT_ID", "").strip()
    if not prebuilt_id:
        raise RuntimeError("PREBUILT_ID env var is required")
    ...
    for _require, dep in _host_items(conanfile):
        ...
        packages.append({
            ...
            "libs": libs,
            "libpath": [pkg_name + "/lib"] if has_lib else [],
            "depends": [],
            "syslibs": syslibs,
